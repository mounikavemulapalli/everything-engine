...
const AdminPanel = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/") // Adjust this URL based on your backend port
      .then(() => console.log("Backend is reachable"))
      .catch((error) => {
        console.error("Error reaching backend:", error);
        alert(`Backend server is not reachable: ${error.message}`);
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/upload-questions", // Backend upload URL
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
      ...
      // Check if the backend response has a success message
      const message =
  ...
  const fetchQuestions = async () => {
    try {
      const response = await axios.get("http://localhost:3000/get-questions"); // Backend URL to fetch questions
      setQuestions(response.data);
    } catch (error) {
      console.error("Error fetching questions:", error);
      alert("Failed to fetch questions.");
